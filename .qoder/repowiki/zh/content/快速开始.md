...
**更新** 客户端配置现在包含了完整的 OAuth2 授权流程支持，包括授权码模式、刷新令牌和客户端凭证模式。
...
- 安全配置
  ...
  - 应用级过滤链（表单登录）
  - 密码编码器 Bean
  - 内存客户端存储 Bean（包含完整的 OAuth2 授权流程配置）
  - 内存用户详情服务 Bean

**更新** 客户端凭证信息现在包含完整的 OAuth2 授权流程支持，包括授权码模式、刷新令牌和客户端凭证模式，以及必要的重定向 URI 和作用域配置。
