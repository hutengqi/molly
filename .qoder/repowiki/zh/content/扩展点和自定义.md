# 扩展点和自定义

<cite>
**本文档引用的文件**
- [MollyUserAccountService.java](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/service/MollyUserAccountService.java)
- [MollyAuthServerAutoConfiguration.java](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/config/MollyAuthServerAutoConfiguration.java)
- [MollyAuthServerProperties.java](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/properties/MollyAuthServerProperties.java)
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports](file://molly-authorization-server-spring-boot-starter/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports)
- [MollyOssAutoConfiguration.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java)
- [MollyOssEndpointAutoConfiguration.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssEndpointAutoConfiguration.java)
- [OssTemplate.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/OssTemplate.java)
- [MollyOssProperties.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/properties/MollyOssProperties.java)
- [AliyunOssTemplate.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/support/aliyun/AliyunOssTemplate.java)
- [MinioOssTemplate.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/support/minio/MinioOssTemplate.java)
- [OssObject.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/OssObject.java)
- [UploadProgress.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/UploadProgress.java)
- [UploadProgressListener.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/UploadProgressListener.java)
- [OssEndpointController.java](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/endpoint/OssEndpointController.java)
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports](file://molly-oss-spring-boot-starter/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports)
- [SecurityConfig.java](file://molly-auth-server-example/src/main/java/cn/molly/example/auth/config/SecurityConfig.java)
- [AuthServerApplication.java](file://molly-auth-server-example/src/main/java/cn/molly/example/auth/AuthServerApplication.java)
- [pom.xml](file://molly-auth-server-example/pom.xml)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介

Molly 框架是一个基于 Spring Boot 的分布式 Web 系统脚手架项目，专注于提供认证授权服务器的完整解决方案。本文档深入解析 Molly 框架的扩展机制，特别是 SPI 接口系统的设计理念和实现方式，重点说明 MollyUserAccountService 接口的作用和扩展点。

框架采用 Spring Boot 自动配置机制，通过条件注解和 Bean 覆盖策略，为开发者提供了灵活的扩展能力。开发者可以轻松替换默认实现，集成自定义功能，满足不同场景下的需求。

**更新** 新增对象存储启动器模块，提供可扩展的存储解决方案，支持阿里云 OSS 和 MinIO 两种存储提供商。

## 项目结构

Molly 项目采用多模块架构设计，主要包含以下模块：

```mermaid
graph TB
subgraph "Molly 项目结构"
Root[Molly 根项目<br/>pom.xml]
subgraph "核心模块"
AuthStarter[molly-authorization-server-spring-boot-starter<br/>认证授权服务器 Starter]
OssStarter[molly-oss-spring-boot-starter<br/>对象存储启动器 Starter]
Example[molly-auth-server-example<br/>示例应用]
Infrastructure[molly-infrastructure<br/>基础设施模块]
end
subgraph "配置文件"
AuthAutoImports[AutoConfiguration.imports<br/>认证自动配置导入]
OssAutoImports[AutoConfiguration.imports<br/>存储自动配置导入]
Properties[配置属性类]
end
Root --> AuthStarter
Root --> OssStarter
Root --> Example
Root --> Infrastructure
AuthStarter --> AuthAutoImports
OssStarter --> OssAutoImports
AuthStarter --> Properties
OssStarter --> Properties
end
```

**图表来源**
- [pom.xml:11-15](file://pom.xml#L11-L15)
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports:1](file://molly-authorization-server-spring-boot-starter/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports#L1)
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports:1](file://molly-oss-spring-boot-starter/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports#L1)

**章节来源**
- [pom.xml:1-81](file://pom.xml#L1-L81)
- [README.md:1-3](file://README.md#L1-L3)

## 核心组件

### MollyUserAccountService 接口

MollyUserAccountService 是框架的核心扩展接口，继承自 Spring Security 的 UserDetailsService：

```mermaid
classDiagram
class UserDetailsService {
<<interface>>
+loadUserByUsername(username) UserDetails
}
class MollyUserAccountService {
<<interface>>
+loadUserByUsername(username) UserDetails
}
UserDetailsService <|-- MollyUserAccountService
```

**图表来源**
- [MollyUserAccountService.java:20](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/service/MollyUserAccountService.java#L20)

该接口的设计理念：
- **统一抽象层**：为 Molly 安全框架提供统一的用户账户服务接口
- **扩展性设计**：支持未来多种认证方式（用户名、手机号、社交媒体等）
- **数据源无关**：实现类可以从数据库、LDAP 等不同数据源加载用户信息

**章节来源**
- [MollyUserAccountService.java:1-22](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/service/MollyUserAccountService.java#L1-L22)

### 对象存储 OssTemplate 接口

**新增** 对象存储模块提供统一的存储抽象接口，支持多种存储提供商：

```mermaid
classDiagram
class OssTemplate {
<<interface>>
+bucketExists(bucket) boolean
+createBucket(bucket) void
+putObject(bucket, objectName, inputStream, size, contentType) OssObject
+getObject(bucket, objectName) InputStream
+removeObject(bucket, objectName) void
+getObjectUrl(bucket, objectName) String
+statObject(bucket, objectName) OssObject
+objectExists(bucket, objectName) boolean
+listObjects(bucket, prefix) OssObject[]
+putObjectIfAbsent(bucket, objectName, inputStream, size, contentType, hash) OssObject
+getThumbnailUrl(bucket, objectName, width, height) String
}
class AliyunOssTemplate {
+resumableUpload(bucket, objectName, filePath, partSize) OssObject
}
class MinioOssTemplate {
+CountingInputStream
}
OssTemplate <|-- AliyunOssTemplate
OssTemplate <|-- MinioOssTemplate
```

**图表来源**
- [OssTemplate.java:16](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/OssTemplate.java#L16)
- [AliyunOssTemplate.java:29](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/support/aliyun/AliyunOssTemplate.java#L29)
- [MinioOssTemplate.java:35](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/support/minio/MinioOssTemplate.java#L35)

**章节来源**
- [OssTemplate.java:1-164](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/OssTemplate.java#L1-L164)

### 自动配置核心类

MollyAuthServerAutoConfiguration 和 MollyOssAutoConfiguration 是框架的核心自动配置类，负责提供默认的 Bean 实现：

```mermaid
classDiagram
class MollyAuthServerAutoConfiguration {
+authorizationServerSettings(properties) AuthorizationServerSettings
+jwkSource() JWKSource
+jwtTokenCustomizer() OAuth2TokenCustomizer
-generateRsa() RSAKey
-generateRsaKey() KeyPair
}
class MollyOssAutoConfiguration {
+ossClient(properties) OSS
+ossTemplate(ossClient, properties) OssTemplate
+minioClient(properties) MinioClient
+ossTemplate(minioClient, properties) OssTemplate
}
class MollyAuthServerProperties {
+issuerUri String
}
class MollyOssProperties {
+provider Provider
+bucket String
+endpointEnabled boolean
+maxFileSize long
+allowedTypes String[]
+thumbnail Thumbnail
+aliyun Aliyun
+minio Minio
}
MollyAuthServerAutoConfiguration --> MollyAuthServerProperties : "使用"
MollyOssAutoConfiguration --> MollyOssProperties : "使用"
```

**图表来源**
- [MollyAuthServerAutoConfiguration.java:54](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/config/MollyAuthServerAutoConfiguration.java#L54)
- [MollyOssAutoConfiguration.java:36](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java#L36)
- [MollyAuthServerProperties.java:16](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/properties/MollyAuthServerProperties.java#L16)
- [MollyOssProperties.java:18](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/properties/MollyOssProperties.java#L18)

**章节来源**
- [MollyAuthServerAutoConfiguration.java:1-161](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/config/MollyAuthServerAutoConfiguration.java#L1-L161)
- [MollyOssAutoConfiguration.java:1-131](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java#L1-L131)
- [MollyAuthServerProperties.java:1-25](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/properties/MollyAuthServerProperties.java#L1-L25)
- [MollyOssProperties.java:1-142](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/properties/MollyOssProperties.java#L1-L142)

## 架构概览

Molly 框架采用分层架构设计，通过 Spring Boot 自动配置机制实现松耦合的扩展架构：

```mermaid
graph TB
subgraph "应用层"
App[业务应用]
Config[自定义配置]
CustomUser[自定义用户服务]
CustomOss[自定义存储实现]
end
subgraph "框架层"
AuthStarter[认证授权服务器 Starter]
OssStarter[对象存储启动器 Starter]
AuthAutoConfig[认证自动配置类]
OssAutoConfig[存储自动配置类]
Interfaces[扩展接口]
Endpoints[HTTP 端点]
end
subgraph "基础设施层"
SpringBoot[Spring Boot]
Security[Spring Security]
OAuth2[OAuth2 授权服务器]
Storage[存储提供商]
End
App --> Config
Config --> AuthStarter
Config --> OssStarter
App --> AuthStarter
App --> OssStarter
AuthStarter --> AuthAutoConfig
OssStarter --> OssAutoConfig
AuthAutoConfig --> Interfaces
OssAutoConfig --> Interfaces
OssAutoConfig --> Endpoints
AuthAutoConfig --> SpringBoot
AuthAutoConfig --> Security
AuthAutoConfig --> OAuth2
OssAutoConfig --> Storage
Interfaces --> Security
Interfaces --> OAuth2
Interfaces --> Storage
Endpoints --> Interfaces
```

**图表来源**
- [MollyAuthServerAutoConfiguration.java:28-50](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/config/MollyAuthServerAutoConfiguration.java#L28-L50)
- [MollyOssAutoConfiguration.java:34-36](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java#L34-L36)
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports:1](file://molly-authorization-server-spring-boot-starter/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports#L1)
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports:1](file://molly-oss-spring-boot-starter/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports#L1)

### Bean 覆盖机制

框架通过 `@ConditionalOnMissingBean` 注解实现智能的 Bean 覆盖机制：

```mermaid
sequenceDiagram
participant App as 应用程序
participant AuthAutoConfig as 认证自动配置类
participant OssAutoConfig as 存储自动配置类
participant Spring as Spring 容器
participant UserBean as 用户自定义 Bean
App->>Spring : 启动应用
Spring->>AuthAutoConfig : 创建认证自动配置类实例
Spring->>OssAutoConfig : 创建存储自动配置类实例
AuthAutoConfig->>Spring : 注册默认用户服务 Bean
OssAutoConfig->>Spring : 注册默认存储 Bean
Spring->>Spring : 检查是否已存在同名 Bean
alt 用户已提供自定义 Bean
Spring->>UserBean : 使用用户提供的 Bean
UserBean-->>Spring : 返回自定义 Bean
else 用户未提供 Bean
Spring->>AuthAutoConfig : 使用默认用户服务实现
Spring->>OssAutoConfig : 使用默认存储实现
AuthAutoConfig-->>Spring : 返回默认用户服务 Bean
OssAutoConfig-->>Spring : 返回默认存储 Bean
end
Spring-->>App : 完成 Bean 注入
```

**图表来源**
- [MollyAuthServerAutoConfiguration.java:67-73](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/config/MollyAuthServerAutoConfiguration.java#L67-L73)
- [MollyOssAutoConfiguration.java:58-86](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java#L58-L86)
- [MollyOssAutoConfiguration.java:107-128](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java#L107-L128)

**章节来源**
- [MollyAuthServerAutoConfiguration.java:67-120](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/config/MollyAuthServerAutoConfiguration.java#L67-L120)
- [MollyOssAutoConfiguration.java:58-128](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java#L58-L128)

## 详细组件分析

### SPI 接口系统设计

Molly 框架的 SPI（Service Provider Interface）接口系统采用简洁而强大的设计理念：

#### 接口层次设计

```mermaid
classDiagram
class UserDetailsService {
<<interface>>
+loadUserByUsername(username) UserDetails
}
class MollyUserAccountService {
<<interface>>
+loadUserByUsername(username) UserDetails
}
class OssTemplate {
<<interface>>
+putObject(bucket, objectName, inputStream, size, contentType) OssObject
+getObject(bucket, objectName) InputStream
+removeObject(bucket, objectName) void
+getThumbnailUrl(bucket, objectName, width, height) String
}
class CustomUserAccountService {
+loadUserByUsername(username) UserDetails
+loadUserByPhone(phone) UserDetails
+loadUserBySocialId(socialId) UserDetails
}
class CustomOssTemplate {
+putObject(bucket, objectName, inputStream, size, contentType) OssObject
+getObject(bucket, objectName) InputStream
+customOperation() void
}
UserDetailsService <|-- MollyUserAccountService
OssTemplate <|-- CustomOssTemplate
MollyUserAccountService <|-- CustomUserAccountService
```

**图表来源**
- [MollyUserAccountService.java:20](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/service/MollyUserAccountService.java#L20)
- [OssTemplate.java:16](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/OssTemplate.java#L16)

#### 设计原则

1. **最小接口原则**：仅定义必要的方法，避免过度设计
2. **向后兼容性**：继承现有 Spring Security 接口，确保兼容性
3. **扩展性**：为未来认证方式预留扩展点
4. **数据源无关**：支持多种用户数据存储方式
5. **统一抽象**：为不同存储提供商提供统一的操作接口

**章节来源**
- [MollyUserAccountService.java:5-19](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/service/MollyUserAccountService.java#L5-L19)
- [OssTemplate.java:6-15](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/OssTemplate.java#L6-L15)

### 自动配置覆盖机制

#### 条件注解详解

框架使用多种条件注解来控制 Bean 的创建和覆盖：

```mermaid
flowchart TD
Start([Bean 注册开始]) --> CheckCondition["检查 @ConditionalOnMissingBean"]
CheckCondition --> BeanExists{"容器中是否存在同名 Bean?"}
BeanExists --> |是| UseExisting["使用现有 Bean"]
BeanExists --> |否| CreateDefault["创建默认 Bean"]
CreateDefault --> CheckConditions["检查其他条件注解"]
CheckConditions --> ConditionsMet{"条件满足?"}
ConditionsMet --> |是| RegisterBean["注册到 Spring 容器"]
ConditionsMet --> |否| SkipRegistration["跳过注册"]
UseExisting --> SkipCreation["跳过创建"]
RegisterBean --> End([完成])
SkipCreation --> End
SkipRegistration --> End
```

**图表来源**
- [MollyAuthServerAutoConfiguration.java:67-73](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/config/MollyAuthServerAutoConfiguration.java#L67-L73)
- [MollyOssAutoConfiguration.java:45-47](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java#L45-L47)

#### Bean 覆盖策略

| Bean 类型 | 默认实现 | 覆盖方式 | 最佳实践 |
|-----------|----------|----------|----------|
| AuthorizationServerSettings | 从配置属性读取 issuerUri | 提供同名 Bean | 确保配置正确 |
| JWKSource | 内存中生成 RSA 密钥对 | 提供安全的密钥源 | 生产环境必须覆盖 |
| OAuth2TokenCustomizer | 添加 authorities 声明 | 提供自定义令牌定制 | 根据业务需求定制 |
| OssTemplate | 阿里云 OSS 实现 | 提供同名 Bean | 确保存储配置正确 |
| OSS 客户端 | 阿里云客户端实例 | 提供同名 Bean | 生产环境必须覆盖 |
| Minio 客户端 | MinIO 客户端实例 | 提供同名 Bean | 确保存储配置正确 |

**章节来源**
- [MollyAuthServerAutoConfiguration.java:67-120](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/config/MollyAuthServerAutoConfiguration.java#L67-L120)
- [MollyOssAutoConfiguration.java:58-128](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java#L58-L128)

### 自定义用户服务实现

#### 实现要求和最佳实践

要实现自定义的用户服务，需要遵循以下要求：

1. **接口实现**：实现 `MollyUserAccountService` 接口
2. **线程安全**：确保实现类是线程安全的
3. **性能优化**：合理使用缓存机制
4. **错误处理**：提供完善的异常处理机制
5. **依赖注入**：正确使用 Spring 依赖注入

#### 生命周期管理

```mermaid
stateDiagram-v2
[*] --> Created : Bean 创建
Created --> Initialized : 初始化
Initialized --> Ready : 准备就绪
Ready --> Used : 服务调用
Used --> Ready : 继续使用
Ready --> Destroyed : 应用关闭
Destroyed --> [*]
state Ready {
[*] --> Caching : 缓存初始化
Caching --> Database : 数据库查询
Database --> Caching : 更新缓存
}
```

**图表来源**
- [MollyUserAccountService.java:13-14](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/service/MollyUserAccountService.java#L13-L14)

**章节来源**
- [MollyUserAccountService.java:13-14](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/service/MollyUserAccountService.java#L13-L14)

### 自定义存储服务实现

**新增** 自定义存储服务实现遵循相同的 SPI 接口设计：

#### 实现要求和最佳实践

要实现自定义的存储服务，需要遵循以下要求：

1. **接口实现**：实现 `OssTemplate` 接口
2. **异常处理**：提供完善的 `OssException` 处理机制
3. **线程安全**：确保实现类是线程安全的
4. **性能优化**：支持大文件分片上传和断点续传
5. **进度回调**：实现 `UploadProgressListener` 支持上传进度监控
6. **依赖注入**：正确使用 Spring 依赖注入

#### 存储提供商适配

```mermaid
classDiagram
class OssTemplate {
<<interface>>
+putObject(bucket, objectName, inputStream, size, contentType) OssObject
+getObject(bucket, objectName) InputStream
+removeObject(bucket, objectName) void
+getThumbnailUrl(bucket, objectName, width, height) String
}
class CustomStorageProvider {
<<interface>>
+connect() void
+disconnect() void
+uploadWithRetry() void
}
class AlibabaCloudOSS {
+resumableUpload() void
+generateThumbnail() String
}
class MinIO {
+countingInputStream() void
+generateThumbnail() String
}
OssTemplate <|-- CustomStorageProvider
OssTemplate <|-- AlibabaCloudOSS
OssTemplate <|-- MinIO
```

**图表来源**
- [OssTemplate.java:16](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/OssTemplate.java#L16)
- [AliyunOssTemplate.java:29](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/support/aliyun/AliyunOssTemplate.java#L29)
- [MinioOssTemplate.java:35](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/support/minio/MinioOssTemplate.java#L35)

**章节来源**
- [OssTemplate.java:1-164](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/OssTemplate.java#L1-L164)
- [AliyunOssTemplate.java:1-272](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/support/aliyun/AliyunOssTemplate.java#L1-L272)
- [MinioOssTemplate.java:1-312](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/support/minio/MinioOssTemplate.java#L1-L312)

## 依赖分析

### Maven 依赖关系

```mermaid
graph TB
subgraph "示例应用依赖"
ExampleApp[molly-auth-server-example]
AuthStarterDep[molly-authorization-server-spring-boot-starter]
OssStarterDep[molly-oss-spring-boot-starter]
SpringWeb[spring-boot-starter-web]
SpringSecurity[spring-boot-starter-security]
end
ExampleApp --> AuthStarterDep
ExampleApp --> OssStarterDep
ExampleApp --> SpringWeb
ExampleApp --> SpringSecurity
AuthStarterDep --> SpringBoot[spring-boot-dependencies]
AuthStarterDep --> Nimbus[nimbus-jose-jwt]
AuthStarterDep --> SpringSecurityOAuth2[Spring Security OAuth2]
AuthStarterDep --> CommonsCollections[Apache Commons Collections4]
OssStarterDep --> SpringBoot
OssStarterDep --> AliyunOSS[阿里云 OSS SDK]
OssStarterDep --> Minio[MinIO SDK]
OssStarterDep --> Jackson[Jackson]
```

**图表来源**
- [pom.xml:16-30](file://molly-auth-server-example/pom.xml#L16-L30)

### Spring Boot 自动配置导入

框架通过 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` 文件声明自动配置类：

**章节来源**
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports:1](file://molly-authorization-server-spring-boot-starter/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports#L1)
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports:1](file://molly-oss-spring-boot-starter/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports#L1)

## 性能考虑

### 缓存策略

为了提高用户服务和存储服务的性能，建议实现以下缓存策略：

1. **用户信息缓存**：缓存用户基本信息，减少数据库查询
2. **权限信息缓存**：缓存用户权限信息，避免重复计算
3. **存储元信息缓存**：缓存对象元信息，减少存储查询
4. **TTL 管理**：设置合理的缓存过期时间
5. **缓存失效**：实现用户信息更新时的缓存失效机制

### 线程安全

```mermaid
classDiagram
class ThreadSafeUserService {
-cache ConcurrentMap~String,UserDetails~
-lock ReadWriteLock
+loadUserByUsername(username) UserDetails
+invalidateCache(username) void
+updateUser(user) void
}
class ThreadSafeOssService {
-cache ConcurrentMap~String,OssObject~
-lock ReentrantReadWriteLock
+putObject(bucket, objectName, inputStream, size, contentType) OssObject
+getObject(bucket, objectName) InputStream
+invalidateCache(key) void
}
class NonThreadSafeUserService {
-cache Map~String,UserDetails~
+loadUserByUsername(username) UserDetails
}
class NonThreadSafeOssService {
-cache Map~String,OssObject~
+putObject(bucket, objectName, inputStream, size, contentType) OssObject
}
ThreadSafeUserService <|-- NonThreadSafeUserService
ThreadSafeOssService <|-- NonThreadSafeOssService
```

**图表来源**
- [MollyUserAccountService.java:20](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/service/MollyUserAccountService.java#L20)
- [OssTemplate.java:16](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/core/OssTemplate.java#L16)

### 异步处理

对于高并发场景，可以考虑异步处理用户查询和文件上传请求：

```mermaid
sequenceDiagram
participant Client as 客户端
participant UserService as 用户服务
participant StorageService as 存储服务
participant Cache as 缓存层
participant DB as 数据库
participant Storage as 存储提供商
Client->>UserService : 查询用户信息
UserService->>Cache : 检查缓存
alt 缓存命中
Cache-->>UserService : 返回用户信息
UserService-->>Client : 返回结果
else 缓存未命中
UserService->>DB : 异步查询数据库
DB-->>UserService : 返回用户信息
UserService->>Cache : 更新缓存
UserService-->>Client : 返回结果
end
Client->>StorageService : 上传文件
StorageService->>Storage : 异步上传到存储
Storage-->>StorageService : 返回上传结果
StorageService-->>Client : 返回结果
```

## 故障排除指南

### 常见问题及解决方案

#### 1. 用户服务 Bean 未被识别

**问题描述**：自定义的用户服务实现没有生效

**解决方案**：
- 确保实现类上有 `@Component` 或 `@Service` 注解
- 确保实现类正确实现了 `MollyUserAccountService` 接口
- 检查包扫描路径是否正确

#### 2. 存储服务 Bean 未被识别

**问题描述**：自定义的存储服务实现没有生效

**解决方案**：
- 确保实现类上有 `@Component` 或 `@Service` 注解
- 确保实现类正确实现了 `OssTemplate` 接口
- 检查包扫描路径是否正确
- 确保存储提供商依赖已正确添加

#### 3. Bean 覆盖不生效

**问题描述**：提供的自定义 Bean 没有覆盖默认实现

**解决方案**：
- 确保自定义 Bean 的名称与默认 Bean 相同
- 检查 Bean 的作用域和生命周期
- 确认 Bean 的优先级设置
- 验证条件注解是否满足

#### 4. 密钥安全问题

**问题描述**：生产环境使用默认密钥导致安全风险

**解决方案**：
- 提供自定义的 `JWKSource` Bean
- 使用安全的密钥存储方案（密钥库、硬件安全模块等）
- 定期轮换密钥

#### 5. 存储连接问题

**问题描述**：存储服务连接失败或认证失败

**解决方案**：
- 检查存储提供商配置是否正确
- 验证网络连接和防火墙设置
- 确认凭据和权限配置
- 查看存储提供商的 API 限制和配额

**章节来源**
- [MollyAuthServerAutoConfiguration.java:81-82](file://molly-authorization-server-spring-boot-starter/src/main/java/cn/molly/security/auth/config/MollyAuthServerAutoConfiguration.java#L81-L82)
- [MollyOssAutoConfiguration.java:60-72](file://molly-oss-spring-boot-starter/src/main/java/cn/molly/oss/config/MollyOssAutoConfiguration.java#L60-L72)

## 结论

Molly 框架通过精心设计的 SPI 接口系统和智能的自动配置机制，为开发者提供了强大而灵活的扩展能力。MollyUserAccountService 接口作为核心扩展点，不仅保持了与 Spring Security 的兼容性，还为未来的认证方式扩展预留了空间。

**更新** 新增的对象存储启动器模块进一步增强了系统的可扩展性，提供了统一的存储抽象接口，支持多种存储提供商的无缝切换。

框架的关键优势包括：

1. **简洁的接口设计**：最小化的接口定义，易于理解和实现
2. **智能的 Bean 覆盖**：通过条件注解实现无缝的自定义覆盖
3. **安全的默认实现**：提供安全的默认配置，同时允许完全自定义
4. **良好的性能特性**：支持缓存和异步处理等性能优化手段
5. **可扩展的存储架构**：统一的存储接口支持多种存储提供商
6. **完善的错误处理**：提供一致的异常处理机制

对于框架扩展开发者，建议遵循本文档的最佳实践，在保证线程安全和性能的前提下，充分利用框架提供的扩展机制来满足特定的业务需求。无论是用户认证还是文件存储，Molly 框架都为开发者提供了清晰的扩展路径和完善的工具支持。

## 附录

### 插件化开发指南

#### 开发步骤

1. **创建扩展模块**：创建独立的 Maven 模块
2. **实现扩展接口**：实现相应的 SPI 接口
3. **配置依赖**：添加必要的依赖项
4. **测试验证**：编写单元测试和集成测试
5. **文档编写**：提供详细的使用文档

#### 最佳实践清单

- ✅ 遵循接口设计原则
- ✅ 实现线程安全
- ✅ 提供完善的错误处理
- ✅ 使用依赖注入
- ✅ 实现缓存机制
- ✅ 编写测试用例
- ✅ 提供配置选项
- ✅ 文档化扩展点

### 配置示例

#### 自定义用户服务配置

```java
@Configuration
public class CustomUserConfig {
    
    @Bean
    public MollyUserAccountService customUserAccountService() {
        return new CustomUserAccountServiceImpl();
    }
}
```

#### 自定义存储服务配置

**新增** 自定义存储服务配置示例：

```java
@Configuration
public class CustomOssConfig {
    
    @Bean
    @Primary
    public OssTemplate customOssTemplate() {
        return new CustomOssTemplateImpl();
    }
    
    @Bean
    public MollyOssProperties customOssProperties() {
        MollyOssProperties properties = new MollyOssProperties();
        properties.setProvider(MollyOssProperties.Provider.ALIYUN);
        properties.setBucket("your-bucket-name");
        properties.getAliyun().setEndpoint("your-oss-endpoint");
        properties.getAliyun().setAccessKeyId("your-access-key");
        properties.getAliyun().setAccessKeySecret("your-secret-key");
        return properties;
    }
}
```

#### 自定义密钥配置

```java
@Bean
public JWKSource<SecurityContext> customJwkSource() {
    // 实现安全的密钥加载逻辑
    return new CustomJwkSource();
}
```

### 存储提供商切换

**新增** 存储提供商切换配置示例：

```yaml
molly:
  oss:
    provider: minio  # 切换到 MinIO
    bucket: my-app-bucket
    endpoint-enabled: true
    max-file-size: 104857600  # 100MB
    allowed-types:
      - image/jpeg
      - image/png
      - application/pdf
    thumbnail:
      enabled: true
      width: 200
      height: 200
    aliyun:
      endpoint: https://oss-cn-hangzhou.aliyuncs.com
      access-key-id: ${ALIYUN_ACCESS_KEY_ID}
      access-key-secret: ${ALIYUN_ACCESS_KEY_SECRET}
    minio:
      endpoint: http://localhost:9000
      access-key: ${MINIO_ACCESS_KEY}
      secret-key: ${MINIO_SECRET_KEY}
```